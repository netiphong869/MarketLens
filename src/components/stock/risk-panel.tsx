...
export function RiskPanel({ analysis }: { analysis: AnalysisResponse }) {
  return (
    <div className={styles.dashboard}>
      ...
      <Card as="section"><h2 className={styles.sectionTitle}>ความเสี่ยงจากข้อมูล</h2><p>Quality {analysis.scores.quality.score}/100 · {analysis.scores.quality.warnings.join(" · ")}</p><span className={styles.source}>แหล่งข้อมูล: {analysis.quote.provenance.provider} · {new Date(analysis.generatedAt).toLocaleString("th-TH")}</span></Card>
    </div>
  );
}
