...
export function ChartPanel({ analysis }: { analysis: AnalysisResponse }) {
  ...
  return <div className={styles.dashboard}><Card as="section"><div className={styles.timeframes} role="group" aria-label="กรอบเวลา">{frames.map((item) => <button key={item.id} className={`${styles.timeframeButton} ${frame === item.id ? styles.timeframeActive : ""}`} onClick={() => setFrame(item.id)} type="button">{item.label}</button>)}</div><div className={styles.chartShell}><StockChart candles={analysis.candles[frame]} dataLabel={isMock ? "ข้อมูลจำลอง" : "ข้อมูลจริง"} /></div></Card><Card as="section"><h2 className={styles.sectionTitle}>{isMock ? "แผนราคาจำลอง" : "แผนราคา"}</h2><div className={styles.metricGrid}><Metric label="แนวรับใกล้" value={support ? `$${support.low}–${support.high}` : "ไม่มีข้อมูล"} /><Metric label="แนวต้านใกล้" value={resistance ? `$${resistance.low}–${resistance.high}` : "ไม่มีข้อมูล"} /><Metric label="สถานะ" value="รอการยืนยัน" /><Metric label="แท่งล่าสุด" value="สัญญาณชั่วคราว" /></div><p className={styles.notice}>โซนราคาเป็น Scenario จาก{isMock ? "ข้อมูลจำลอง" : "ข้อมูลที่ระบบคำนวณ"} ไม่ใช่คำสั่งซื้อขาย</p></Card><Card as="section"><h2 className={styles.sectionTitle}>Indicator Snapshot</h2><div className={styles.metricGrid}><Metric label="EMA" value="ดูจากคะแนนเทคนิค" /><Metric label="RSI14" value="ดูจากคะแนนเทคนิค" /><Metric label="MACD" value="ดูจากคะแนนเทคนิค" /><Metric label="Volume" value="ดูจากคะแนนเทคนิค" /></div></Card></div>;
}
